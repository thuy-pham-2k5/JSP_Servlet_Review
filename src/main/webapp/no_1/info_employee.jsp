    ...
    <div class="div-parent">
        ...
        <div class="form-container">
            <form action="/employees?action=update&id=${employee.id}">
                ...
                <div class="mb-3">
                    <label for="2" class="form-label">Department</label>
                    <input type="text" name="department" class="form-control"
                           id="2" ${param.action == 'view' ? 'readonly' : ''} value="${employee.department}">
                </div>
                <div class="mb-3">
                    <label for="3" class="form-label">Position</label>
                    <input type="text" name="position" class="form-control"
                           id="3" ${param.action == 'view' ? 'readonly' : ''} value="${employee.position}">
                </div>
                <div class="mb-3">
                    <label for="4" class="form-label">Salary</label>
                    <input type="number" name="salary" class="form-control"
                           id="4" ${param.action == 'view' ? 'readonly' : ''} value="${employee.salary}">
                </div>
                <div class="mb-3">
                    <label for="5" class="form-label">Address</label>
                    <input type="text" name="address" class="form-control"
                           id="5" ${param.action == 'view' ? 'readonly' : ''} value="${employee.address}">
                </div>
